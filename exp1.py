...
CHANGES IN THIS VERSION (logic untouched, only run/reporting changed):
  * GPU is now required -- the script aborts with a clear error if CUDA
    isn't available, instead of silently falling back to CPU.
  * At startup the user is prompted to pick exactly ONE dataset:
        1 -> MNIST-2
        2 -> FashionMNIST-2
    Only that dataset is run (no more looping over both).
  * Both training loops (optimize / optimize_and_prune) now time every
    step and print/return results in the exact requested format:

        training without pruning:
          step 1: accuracy, loss, time
          ...
          step N: accuracy, loss, time
          cumulative result: accuracy, loss, time

        training with pruning:
          step 1: accuracy, sparsity, loss, time
          ...
          step N: accuracy, sparsity, loss, time
          cumulative result: accuracy, sparsity, loss, time

    "accuracy"/"loss" per step = TRAINING accuracy/loss for that step
    (that's what's available step-by-step); "time" per step = wall-clock
    seconds for that step. The "cumulative result" row reports the final
    step's accuracy/loss(/sparsity) plus the TOTAL wall-clock time for
    the whole loop -- i.e. the same numbers Table I ultimately reports,
    with the timing rolled up.
...
# ----------------------------------------------------------------------
# Reporting helpers -- exact output format requested
# ----------------------------------------------------------------------
...
def optimize_and_prune(qckt, iparams, X_train, y_train, X_val, y_val,
    ...
    for t in range(steps):
        ...
        # FIX: re-apply the FULL cumulative frozen mask every step, not just
        # the most recently pruned round's indices. Previously this only
        # re-zeroed a single `idx` variable that got overwritten each
        # pruning round, so earlier frozen params drifted back to nonzero
        # under RMSprop's per-step updates.
        enforce_frozen(params, frozen_mask, param_shape)
        ...
        if t != 0 and t % win_sz == 0:
            idx = build_saliency(grad_buffer, tau_prime)
...
# Loop over shots settings is preserved as an option below.
# ----------------------------------------------------------------------
...
if __name__ == "__main__":
    ...
    with open(f"{args.save}_{dataset_name}_summary.txt", "w") as f:
