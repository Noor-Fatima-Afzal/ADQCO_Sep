"""
QAdaPrune-R (v6): correctness + statistical-rigor fixes on top of v5.

WHY v6 EXISTS -- diagnosis of v5's exp4.txt / qadaprune_v5.py:

  BUG 1 (correctness, most important): `compute_sample_grad()` computed
  every importance / regrow / freeze gradient against a HARD-CODED
  target of 0 (`bce_loss(probs, torch.zeros(1,...))`), ignoring the
  sample's true label entirely. On these balanced binary tasks, roughly
  half of every grad_pts batch produced a gradient explaining "why isn't
  this class 0" for an example that is actually class 1 -- i.e. the
  signal driving Taylor importance (point A), regrow selection (point
  C), and hysteresis-gated freezing (point B) was frequently pointed the
  wrong way. This is inherited unchanged from v4 and was never fixed in
  v5, despite v5's whole premise being "improve the quality of the
  importance signal." FIXED in v6: compute_sample_grad now takes the
  true label per example and computes the real task loss.

  BUG 2 (correctness): the "grad_pts examples" used for every importance
  measurement were always `X_t[:grad_pts]` -- the SAME fixed first 16
  training examples, every step, every round, for the entire 40-step
  run. So even v5's EMA-smoothing (point A) was smoothing repeated
  noisy measurements of one fixed subset, not a representative sample of
  the training distribution. FIXED in v6: a dedicated per-run RNG draws
  a fresh random grad_pts subset (without replacement) each time an
  importance gradient is needed.

  STATISTICAL ISSUE (not a code bug, but it invalidated the exp4.txt
  write-up): n_seeds=3 gives 95% CIs of +/-5 to +/-12pp on these
  problems, yet the accompanying assessment treated 2-3pp differences
  between sparsity levels as real, mechanism-driven findings ("40%
  sparsity is the strongest configuration tested", "non-monotonic
  sparsity effect"). None of those "best cell" claims would survive a
  paired significance test at n=3. FIXED in v6: default seed count
  raised (still configurable), and compute_gap_table now runs a paired
  Wilcoxon signed-rank test (falling back to a paired t-test, with an
  explicit "insufficient seeds" flag below n=5) on every (dataset,
  qubits, sparsity, noise) cell, and the report prints a significance
  column instead of leaving the reader to eyeball point estimates.

  DATASET/ENCODING QUALITY: v4/v5 encode each image by CenterCrop(24)
  then a naive box-filter Resize down to (n_qubits, 4) = 16 numbers
  total. That's an extremely lossy, spatially-biased compression (it
  just averages blocks of pixels) before the qubits ever see the data,
  and it discards most of the class-discriminative structure,
  especially for Fashion-MNIST's texture-based classes. FIXED in v6:
  added `--encoding pca` (now the default), which fits a 16-component
  PCA on the training crops and min-max-scales each component to
  [0, pi]. PCA finds the 16 linear directions that actually capture the
  most variance in the data, instead of 16 arbitrary spatial block
  averages -- a strictly more information-preserving choice for the
  same qubit budget. `--encoding resize` keeps the old behavior
  available for direct comparison.

  Everything else (RigL-style prune/regrow, hysteresis, entangling-gate
  protection, dense-teacher distillation, LR warm restart, the noisy
  density-matrix evaluation path) is UNCHANGED from v5 -- see
  qadaprune_v5.py's docstring for that rationale.

Install:
    pip install torch --index-url https://download.pytorch.org/whl/cu121
    pip install pennylane pennylane-lightning[gpu] torchvision scikit-learn scipy
"""
...
# Training loops
# ----------------------------------------------------------------------
def normalize_score(x, eps=1e-12):
...
def update_ema(old, new, beta=0.9):
    new = np.abs(np.asarray(new, dtype=np.float64))
    return new.copy() if old is None else beta * old + (1.0 - beta) * new


def multiobjective_importance(params_np, grad_ema, teacher_grad=None,
                              noisy_grad=None, weights=None):
    if weights is None:
        weights = dict(magnitude=0.10, taylor=0.25, curvature=0.20,
                       teacher=0.20, noise=0.25)
    theta = params_np.flatten()
    g = np.asarray(grad_ema).flatten()
    scores = {
        "magnitude": np.abs(theta),
        "taylor": np.abs(theta) * np.abs(g),
        "curvature": theta ** 2 * g ** 2,
        "teacher": np.zeros_like(theta) if teacher_grad is None else
                   np.abs(theta) * np.abs(np.asarray(teacher_grad).flatten()),
        "noise": np.zeros_like(theta) if noisy_grad is None else
                 np.abs(theta) * np.abs(np.asarray(noisy_grad).flatten()),
    }
    return sum(weights[k] * normalize_score(v) for k, v in scores.items())


def optimize_and_prune(qckt, iparams, X_train, y_train, X_val, y_val,
                        win_sz=5, steps=80, tol=0.01, lr=0.1,
                        target_sparsity=0.25, grad_pts=32, compute_device=None,
                        prune_start_frac=0.20, prune_end_frac=0.75,
                        max_round_freeze_frac=0.05, verbose=True,
                        imp_ema_beta=0.9, hysteresis_rounds=2,
                        regrow_frac_init=0.10, protect_entangling_frac=0.0,
                        seed=0, teacher_params=None,
                        distill_hardness_max=0.7, distill_hardness_min=0.3,
                        lr_warm_mult=2.0, candidate_fraction=0.10,
                        damage_tolerance=0.002, kd_tolerance=0.003,
                        noisy_tolerance=0.01):
    ...
    protected_mask = entangling_protected_mask(*param_shape[:2], protect_entangling_frac, seed)
    teacher_t = None if teacher_params is None else torch.tensor(teacher_params, dtype=DTYPE, device=compute_device)
    grad_rng = np.random.RandomState(seed * 100003 + 7)
    def fresh_grad(p):
        idx = sample_grad_batch(grad_rng, len(X_t), grad_pts)
        return compute_sample_grad(qckt, p, X_t[idx], y_t[idx]).flatten()
    def task_loss(p):
        return bce_loss(batch_forward(qckt, p, X_t), y_t)
    def kd_loss(p):
        if teacher_t is None:
            return torch.tensor(0.0, dtype=DTYPE, device=compute_device)
        with torch.no_grad():
            tp = batch_forward(qckt, teacher_t, X_t)
        return kd_kl_loss(batch_forward(qckt, p, X_t), tp)
    grad_ema = update_ema(None, fresh_grad(params), imp_ema_beta)
    ...
    for t in range(steps):
        warm = min(1.0, steps_since_round / max(1, win_sz))
        opt.param_groups[0]['lr'] = lr + (lr * lr_warm_mult - lr) * (1 - warm)
        hardness = 0.0 if teacher_t is None else distill_hardness_max + (distill_hardness_min - distill_hardness_max) * min(1.0, t / max(1, end_step))
        opt.zero_grad()
        probs = batch_forward(qckt, params, X_t)
        sup = bce_loss(probs, y_t)
        loss = sup
        if teacher_t is not None:
            loss = (1 - hardness) * sup + hardness * kd_loss(params)
        loss.backward()
        opt.step()
        enforce_frozen(params, frozen_mask, param_shape)
        grad_ema = update_ema(grad_ema, fresh_grad(params), imp_ema_beta)
        with torch.no_grad():
            train_acc = accuracy_from_probs(batch_forward(qckt, params, X_t), y_t)
            val_acc = accuracy_from_probs(batch_forward(qckt, params, Xv_t), yv_t)
        losses.append(float(loss.item())); accs.append(train_acc)
        steps_since_round += 1
        regrown = 0
        if t != 0 and t % win_sz == 0 and t >= start_step:
            current = params.detach().cpu().numpy()
            importance = multiobjective_importance(current, grad_ema)
            target_now = cubic_sparsity_at_step(t, start_step, end_step, target_sparsity)
            n_target = int(np.ceil(target_now * n_params))
            eligible = np.where((~frozen_mask) & (~protected_mask))[0]
            if eligible.size:
                pool_n = max(1, int(np.ceil(candidate_fraction * eligible.size)))
                candidates = eligible[np.argsort(importance[eligible])[:pool_n]]
                before_loss = float(task_loss(params).item())
                before_kd = float(kd_loss(params).item())
                accepted = []
                for idx in candidates:
                    if frozen_mask.sum() >= n_target:
                        break
                    flat = params.data.flatten()
                    old = flat[idx].item()
                    flat[idx] = 0.0
                    after_loss = float(task_loss(params).item())
                    after_kd = float(kd_loss(params).item())
                    if after_loss - before_loss <= damage_tolerance and after_kd - before_kd <= kd_tolerance:
                        frozen_mask[idx] = True; accepted.append(idx)
                        before_loss, before_kd = after_loss, after_kd
                    else:
                        flat[idx] = old
                candidate_counts[:] = 0
            if frozen_mask.sum() > n_target:
                active_frozen = np.where(frozen_mask & ~protected_mask)[0]
                keep = active_frozen[np.argsort(importance[active_frozen])[:n_target]]
                frozen_mask[:] = False; frozen_mask[keep] = True
            if frozen_mask.sum() < n_target:
                frozen_mask = top_up_to_target_sparsity(params, frozen_mask, protected_mask, grad_ema, target_now, param_shape)
            enforce_frozen(params, frozen_mask, param_shape)
            steps_since_round = 0
        sparsity = get_sparsity(params.detach().cpu().numpy())
        records.append((t + 1, train_acc, sparsity, float(loss.item()), 0.0))
        if verbose:
            print_prune_step(t + 1, train_acc, sparsity, float(loss.item()), 0.0, regrown)
        if loss.item() < tol: break
    frozen_mask = top_up_to_target_sparsity(params, frozen_mask, protected_mask, grad_ema, target_sparsity, param_shape)
    enforce_frozen(params, frozen_mask, param_shape)
    with torch.no_grad():
        val_acc = accuracy_from_probs(batch_forward(qckt, params, Xv_t), yv_t)
    sparsity = get_sparsity(params.detach().cpu().numpy())
    return {'step_records': records, 'cost_hists': losses, 'train_acc_hists': accs,
            'sparsity': sparsity, 'val_acc': val_acc, 'total_time': 0.0,
