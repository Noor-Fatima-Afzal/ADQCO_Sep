"""
  ...
  * Dataset:  MNIST (digits 3 vs 6) / FashionMNIST (dress vs shirt).
              Images center-cropped to 24x24, downsampled to 4x4.
              500 training images, 300 held-out validation images
              (split evenly across the two classes), matching the paper's
              "500 front images ... 300 randomly chosen validation" setup.
  * Circuit:  data-encoding sub-circuit applies RY, RX, RZ, RY per qubit
              using one image row (4 pixel values) per qubit (Fig. 2),
              followed by a variational sub-circuit of alternating RZZ
              gates (via qml.IsingZZ) and a layer of RY rotations.
              Measurement: Pauli-Z on all 4 qubits -> 4-valued output,
              reduced to a 2-valued output by summing the first two and
              last two components (the reduction from [14] the paper
              cites), then softmax -> class probabilities.
  * Loss:     Binary cross-entropy (Eq. 6), not MSE.
  * Metric:   VALIDATION accuracy on the held-out 300-image set at the end
              of training -- this is what Table I reports, not training
              accuracy on the fit set.
  * Pruning target: --target-sparsity (default 0.25) so the run reliably
    prunes "at least 25%" of parameters like the paper reports, instead of
    letting the window/threshold schedule land wherever it happens to.

CHANGES IN v3 (pruning STRATEGY improved -- everything else, including
every print statement's format, is unchanged):

  The v2 run showed exactly the classic failure mode of one-shot /
  threshold-based freezing: at step 11 the run jumped from 5/24 (20.8%)
  to 8/24 (33.3%) frozen parameters in a SINGLE round, training accuracy
  collapsed 0.96 -> 0.72, and with only 9 steps left it only recovered to
  0.89 -- leaving a 6-point validation gap vs. the unpruned run (0.9233
  vs 0.8700). Two changes fix this, both standard in the pruning
  literature:

  1. GRADUAL / CUBIC SPARSITY SCHEDULE (Zhu & Gupta, 2017, "To prune, or
     not to prune"; the same schedule used by GMP*, MAP, GraNet, and most
     modern iterative-pruning baselines). Instead of jumping straight to
     whatever a fixed per-round threshold happens to produce, the target
     sparsity at step t is ramped smoothly:

         s_t = s_f + (s_i - s_f) * (1 - (t - t0) / (t1 - t0))**3

     for t0 <= t <= t1, reaching s_f (target_sparsity) exactly at t1.
     Crucially t1 is set to a FRACTION of total steps (--prune-end-frac,
     default 0.65) rather than the final step -- so pruning finishes
     early and the remaining steps are a pure fine-tuning/recovery tail,
     which GMP*'s ablations show is the single biggest lever for closing
     the pruned-vs-dense accuracy gap at a given sparsity.

  2. TAYLOR (MAGNITUDE x GRADIENT) IMPORTANCE instead of raw
     gradient-magnitude thresholding (Molchanov et al., 2016; the same
     family of sensitivity-based score used by Movement Pruning, Sanh et
     al. 2020). A parameter with a large current value but a small
     *instantaneous* gradient can still matter a lot; scoring by
     |gradient| * |weight| avoids freezing it just because Algorithm 1's
     raw-gradient threshold happened to flag it.

  On top of that, each pruning round is capped at a small maximum number
  of newly-frozen parameters (--max-round-freeze-frac, default 0.08 of
  all parameters per round) so no single round can reproduce the
  3-parameter, 12-point-accuracy shock seen in the v2 log -- sparsity
  increases in small, recoverable increments instead.

  top_up_to_target_sparsity() is kept as a safety net at the very end
  (in case the capped rounds under-shoot target_sparsity by the last
  step), but with the schedule above it should rarely need to do
  anything since the cubic curve already reaches target_sparsity by
  t1 < steps.
